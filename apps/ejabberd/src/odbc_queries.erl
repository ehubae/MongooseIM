...
set_availability(LUsername,LServer, Availability) ->
  ejabberd_odbc:sql_transaction(
    LServer,
    fun() ->
...
get_availability(LServer, Username) ->
  ejabberd_odbc:sql_query(
    LServer,
    [<<"select availability from user_availability "
    "where username='">>, Username, "' and availability = 'Offline' ;"]).


get_group_name(LServer,GroupId) ->
  ejabberd_odbc:sql_query(
    LServer,
    [<<"select muc_light_config.val from  muc_light_config inner join muc_light_rooms on muc_light_config.room_id=muc_light_rooms.id where muc_light_rooms.luser = '">>, GroupId, "' limit 1;"]).

get_registered_contacts(Username,LServer,CONTACT)->
  ejabberd_odbc:sql_query(
    LServer,
    [<<"select username from users where RIGHT(username,7) IN (">>,CONTACT,");"]).

get_members(LServer, Username) ->
  ejabberd_odbc:sql_query(
    LServer,
    [<<"select user_availability.username from user_availability inner join muc_light_occupants on muc_light_occupants.luser = user_availability.username inner join muc_light_rooms on muc_light_rooms.id = muc_light_occupants.room_id and user_availability.availability = 'Offline' and muc_light_rooms.luser = '">>, Username, "';"]).
